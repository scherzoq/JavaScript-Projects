...
// Disable the stop button since not needed until game start.
...
function startGame() {
    ...
    var btn = document.getElementById('btnStop');
    stopEnabled(btn); // enable the start button since the game is now afoot
...
// this function styles the stop button while it is enabled
...
// this function styles the start button while it is enabled
...
function determineAvatar() {
    ...
    var avatarArray = getAvatars(); // return an array of both player's assigned avatars
...
// as squares are selected they check in with this function to see if that partciular
...
// call this function to check board state for any ties
function check4Tie() {
    var boardState = document.getElementById('boardState').innerHTML;
...
function winSound() {
    ...
    setTimeout(function() {sound.currentTime = 0;}, 2000)
}
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { // if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty then the square is unoccupied
            ...
            squareSound(); // play a game sound when the aatar is placed
        }
    }
}
